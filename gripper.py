...
from robot import RobotParam, Robot
...
class EndEffecter:
    ...
    def draw(self, ax, trans):
        self.origin = tr.trans2pos(trans)
        pos1 = self.origin
        pos2 = [self.com_pos[0] * 2, self.com_pos[1] * 2, self.com_pos[2] * 2]
        pos2 = tr.trans2rot(trans) @ pos2 + pos1

        self.draw_censor_power(trans, ax)
        tmp = ((tr.trans2rot(trans) @ [0, 0, 1]) * self.AXIS_LENGTH) 
        ax.quiver(self.origin[0], self.origin[1], self.origin[2], tmp[0],tmp[1],tmp[2], color='red')
        tmp = ((tr.trans2rot(trans) @ [0, 1, 0]) * self.AXIS_LENGTH) 
        ax.quiver(self.origin[0], self.origin[1], self.origin[2], tmp[0],tmp[1],tmp[2], color='blue')
        tmp = ((tr.trans2rot(trans) @ [1, 0, 0]) * self.AXIS_LENGTH) 
        ax.quiver(self.origin[0], self.origin[1], self.origin[2], tmp[0],tmp[1],tmp[2], color='green')
        ...
        trans = np.array([[rotation_matrix[0][0], rotation_matrix[0][1], rotation_matrix[0][2], pos_center[0]],
                          [rotation_matrix[1][0], rotation_matrix[1][1], rotation_matrix[1][2], pos_center[1]],
                          [rotation_matrix[2][0], rotation_matrix[2][1], rotation_matrix[2][2], pos_center[2]],
                          [0, 0, 0, 1]])

        rd.draw_cylinder(radius=0.9, height=length, num_slices=20, color='gray', ax=ax, trans=trans)

        pin_length = length / 5
        trans1 = trans @ tr.get_trans_mat(0.45, 0, (length + pin_length) / 2) @ tr.get_rot_mat('z', np.pi / 4)
        trans2 = trans @ tr.get_trans_mat(-0.45, 0, (length + pin_length) / 2) @ tr.get_rot_mat('z', np.pi / 4)

        rd.draw_cylinder(radius=0.3, height=pin_length, num_slices=4, color='black', ax=ax, trans=trans1)
        rd.draw_cylinder(radius=0.3, height=pin_length, num_slices=4, color='black', ax=ax, trans=trans2)
