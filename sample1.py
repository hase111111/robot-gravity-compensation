...
from gripper import EndEffecter
import trans as tr
import math
...
def make_robot_param() -> RobotParam:
    param = RobotParam()
...
def draw_table(ax, robot: Robot, end_effecter: EndEffecter) -> None:
    # ロボットの手先の位置を取得
    pos = robot.get_joint_pos(2)
    coord = tr.trans2rot(robot.get_joint_trans(2))
    com_pos = [end_effecter.com_pos[2], end_effecter.com_pos[1], end_effecter.com_pos[0]]
    power = end_effecter.get_censor_power(coord)
    moment = np.cross(com_pos, power)
    data = {
        '手先の位置 [m]': "x:{:.3f}, y:{:.3f}, z:{:.3f}".format(*pos),
        '手先からグリッパの重心へ向かうベクトル [m]': "x:{:.3f}, y:{:.3f}, z:{:.3f}".format(*com_pos),
        'グリッパの質量 [kg]': "{:.3f}".format(end_effecter.mass),
        '手先の姿勢(回転行列)': "{:.3f}, {:.3f}, {:.3f} \n {:.3f}, {:.3f}, {:.3f} \n {:.3f}, {:.3f}, {:.3f}".format(*coord[0], *coord[1], *coord[2]),
