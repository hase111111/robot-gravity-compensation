...
class RobotParam:
    class LinkParam:
        ...
        def get_trans_mat(self):
            ans = (tr.get_rot_mat('z', self.theta)
                   @ tr.get_trans_mat(0, 0, self.d)
                   @ tr.get_trans_mat(self.a, 0, 0)
                   @ tr.get_rot_mat('x', self.alpha))
    ...
    def add_link(self, a, alpha, d, theta):
        self.link.append(self.LinkParam(a, alpha, d, theta))
...
class Robot:
    ...
    def get_joint_pos(self, i):
        return tr.trans2pos(self.get_joint_trans(i))

    def draw(self, ax):
        for i in range(len(self._param.link) - 1):
            rd.draw_cylinder(radius=1.5, height=3.0, num_slices=20, color='blue',
                             ax=ax, trans=self.get_joint_trans(i))
            self.draw_link(ax, self.get_joint_pos(i), self.get_joint_pos(i + 1))

        self.draw_link(ax, self._origin, self.get_joint_pos(0))
        rd.draw_cylinder(radius=3, height=2.0, num_slices=20, color='green', ax=ax)
    ...
    def draw_link(self, ax, pos1, pos2):
        ...
        rd.draw_cylinder(radius=1, height=length, num_slices=20, color='red', ax=ax, trans=trans)       
